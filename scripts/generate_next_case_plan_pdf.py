...
SOURCE = ROOT / "docs" / "next-case-study-plan.md"
OUT = ROOT / "docs" / "next-case-study-plan.pdf"
...
class Pdf:
    ...
    def heading(self, value: str, level: int) -> None:
        if level == 1:
            self.text(value, size=22, chars=52)
        elif level == 2:
            self.y += 6
            self.text(value, size=16, chars=66)
        else:
            self.text(value, size=12, chars=76)
